...
export function provinceMatches(admin1, provinceCode) {
  const code = resolveProvinceCode(provinceCode);
  if (!code) return true;

  const expected = PROVINCE_CODES[code];
  const actual = String(admin1 || "").trim().toLowerCase();
  if (!actual) return false;

  return (
    actual === expected.toLowerCase() ||
    actual.includes(code.toLowerCase()) ||
